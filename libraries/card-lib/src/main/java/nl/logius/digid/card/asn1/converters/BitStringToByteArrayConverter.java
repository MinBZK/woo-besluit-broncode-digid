...
public class BitStringToByteArrayConverter extends SimpleConverter<byte[]> {
    @Override
    public byte[] deserialize(Asn1ObjectInputStream in) {
        final int padBits = in.read();
        if (padBits != 0) {
            throw new Asn1Exception("Can only convert full octets bit strings");
        }
        return in.readRemaining();
    }
