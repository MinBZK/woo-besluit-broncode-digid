...
public class SecurityFactoryImpl implements SecurityFactory {
    /**
     * Generates a private key from the EC paramater spec
     *
     * @param params the EC paramters
     * @return private key
     */
    public EcPrivateKey generateKey(ECDomainParameters params) {
        final ECKeyPairGenerator kpg = new ECKeyPairGenerator();
        kpg.init(new ECKeyGenerationParameters(params, CryptoUtils.RANDOM));
        return new EcPrivateKey((ECPrivateKeyParameters) kpg.generateKeyPair().getPrivate());
    }
}
