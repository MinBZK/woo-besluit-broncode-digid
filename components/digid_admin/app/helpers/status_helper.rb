...
module StatusHelper
  ...
  def highlight(text, setting)
    setting ? "<i>#{text&.html_safe}</i>" : text
  end
  ...
  def render_from(date, active=true, text=nil)
    if date.present?
      ...
      raw highlight("#{text} #{I18n.l(date.to_date, format: :default)}", setting)
    else
      text || "-"
    end
  end
  ...
  def render_organization_role(object)
    ...
    if active?(object)
      ...
      html << "#{highlight(t("services.inactive"), true)}"
    end
    html = " (#{html})" unless html.blank?
    html = "#{object.type}#{html}"
    raw html
  end
  ...
  def render_organization_role_type(object)
    ...
    raw highlight(object.type, setting)
  end

  def render_expiration_date(date)
    return "-" if date.nil?

    date < DateTime.now && red_color(I18n.l(date.to_date, format: :default)).html_safe ||
    expiration_date >= date && orange_color(I18n.l(date.to_date, format: :default)).html_safe ||
    I18n.l(date.to_date, format: :default)
  end

  def red_color(text)
    "<span class='red-background'>#{text}</span>"
  end

  def orange_color(text)
    "<span class='orange-background'>#{text}</span>"
  end

  private
  def expiration_date
    Configuration.get_int("Waarschuwingstermijn_te_verlopen_certificaten").days.from_now.to_date
  end
end
