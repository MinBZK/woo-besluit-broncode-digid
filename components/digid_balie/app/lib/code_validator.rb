...
class CodeValidator < ActiveModel::EachValidator
  def initialize(options)
    if options[:starts_with].blank? || (options[:starts_with] !~ /\A[A-Z]\z/)
      fail ArgumentError, 'Invalid starts_with option'
    end
    super
  end

  def validate_each(object, attribute, value)
    return if value.nil? || value.upcase =~ /\A#{options[:starts_with]}/
