...
class SwitchIntegrationTest {
    ...
    @Test
    void getAllSwitchesTest() throws URISyntaxException {
        RequestEntity<Map<String, String>> re = new RequestEntity(headers, HttpMethod.GET, new URI("/iapi/app_switches.json"));
    ...
    @Test
    void getSwitchTest() throws URISyntaxException {
        RequestEntity<Map<String, String>> re = new RequestEntity(headers, HttpMethod.GET, new URI("/iapi/app_switches/1.json"));
        ResponseEntity<String> response = testRestTemplate.exchange(re, String.class);
        assertEquals(HttpStatus.OK, response.getStatusCode());
    ...
    @Test
    void updateSwitchTest() throws URISyntaxException, JSONException {
        Switch sw = repository.findById(1L).get();
        JSONObject params = new JSONObject();
        params.put("name", "test");
        params.put("description", "test2");
        params.put("status", SwitchStatus.INACTIVE);
        assertNotEquals("test", sw.getName());
        assertNotEquals("test2", sw.getDescription());
        assertNotEquals(SwitchStatus.INACTIVE, sw.getStatus());
        RequestEntity<Map<String, String>> re = new RequestEntity(params.toString(), headers, HttpMethod.PUT, new URI("/iapi/app_switches/1.json"));
        ResponseEntity<String> response = testRestTemplate.exchange(re, String.class);
        assertEquals(HttpStatus.NO_CONTENT, response.getStatusCode());
        assertEquals("test", repository.findById(1L).get().getName());
        assertEquals("test2", repository.findById(1L).get().getDescription());
        assertEquals(SwitchStatus.INACTIVE, repository.findById(1L).get().getStatus());
    }
}
