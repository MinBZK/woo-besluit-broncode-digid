...
/**
 * Used before starting a nl.logius.digid.app.domain.shared.flow
 */
public class UndefinedFlow extends Flow {
    public static final String NAME = "undefined_flow";

    private static final Map<BaseState, Map<BaseAction, BaseState>> TRANSITIONS_START_ACTION_RESULT = Map.of(
        State.INITIALIZED, Map.of(Action.CONFIRM_PASSWORD, State.PASSWORD_CONFIRMED)
    );

    public UndefinedFlow(AppAuthenticatorService appAuthenticatorService, DigidClient digidClient, NsClient nsClient, AppSessionService appSessionService, FlowFactory flowFactory) {
