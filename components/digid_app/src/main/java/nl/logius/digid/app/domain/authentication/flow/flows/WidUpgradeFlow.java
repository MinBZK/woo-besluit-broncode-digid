...
public class WidUpgradeFlow extends AuthenticationFlow {
    public static final String NAME = "wid_upgrade_flow";

    private static final Map<BaseState, Map<BaseAction, BaseState>> TRANSITIONS_START_ACTION_RESULT = Map.of(
        State.INITIALIZED, Map.of(Action.WID_CHALLENGE, State.WID_CHALLENGED),
        State.WID_CHALLENGED, Map.of(Action.WID_UPGRADE, State.VERIFIED)
    );

    public WidUpgradeFlow(AppAuthenticatorService appAuthenticatorService, DigidClient digidClient, NsClient nsClient, AppSessionService appSessionService, FlowFactory flowFactory) {
