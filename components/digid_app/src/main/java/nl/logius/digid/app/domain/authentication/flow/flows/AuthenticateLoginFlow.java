...
public class AuthenticateLoginFlow extends AuthenticationFlow {
    public static final String NAME = "authenticate_login";
    ...
    private static final Map<BaseState, Map<BaseAction, BaseState>> TRANSITIONS_START_ACTION_RESULT = Map.of(
        State.INITIALIZED, Map.of(
            Action.INITIALIZE, State.INITIALIZED,
            Action.GET_STATUS, State.INITIALIZED,
            Action.CHALLENGE, State.RETRIEVED
        ),
        State.RETRIEVED, Map.of(
            Action.GET_STATUS, State.RETRIEVED,
            Action.AUTHENTICATE, State.AUTHENTICATED
        ),
        State.AUTHENTICATED, Map.of(Action.GET_STATUS, State.AUTHENTICATED)
    );

    public AuthenticateLoginFlow(AppAuthenticatorService appAuthenticatorService, DigidClient digidClient, NsClient nsClient, AppSessionService appSessionService, FlowFactory flowFactory) {
