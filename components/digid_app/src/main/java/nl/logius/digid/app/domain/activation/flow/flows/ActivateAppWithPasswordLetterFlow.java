...
public class ActivateAppWithPasswordLetterFlow extends ActivationFlow {
    public static final String NAME = "activate_app_with_password_letter_flow";

    private static final Map<BaseState, Map<BaseAction, BaseState>> TRANSITIONS_START_ACTION_RESULT = Map.of(
        State.PASSWORD_CONFIRMED, Map.of(Action.CONFIRM_SESSION, State.SESSION_CONFIRMED),
        State.SESSION_CONFIRMED, Map.of(Action.SEND_LETTER, State.LETTER_SENT),
        State.LETTER_SENT, Map.of(Action.POLL_LETTER, State.LETTER_POLLING),
        State.LETTER_POLLING, Map.of(Action.CHALLENGE, State.CHALLENGED),
    ...
    public ActivateAppWithPasswordLetterFlow(AppAuthenticatorService appAuthenticatorService, DigidClient digidClient, NsClient nsClient, AppSessionService appSessionService, FlowFactory flowFactory) {
    ...
    public AppResponse activateApp(AppAuthenticator appAuthenticator, AppSession appSession) {
        digidClient.remoteLog("1484", Map.of(
