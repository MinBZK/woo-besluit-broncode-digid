...
public class ReplaceExistingApplication extends AbstractFlowStep<ReplaceApplicationRequest> {
    private final DigidClient digidClient;

    public ReplaceExistingApplication(DigidClient digidClient) {
        super();
        this.digidClient = digidClient;
    }

    @Override
    public AppResponse process(Flow flow, ReplaceApplicationRequest request) {
        if (appSession.getRegistrationId() == null) {
            return new NokResponse();
        }

        Map<String, String> result = digidClient.replaceExistingApplication(appSession.getRegistrationId(), request.isReplaceApplication());
