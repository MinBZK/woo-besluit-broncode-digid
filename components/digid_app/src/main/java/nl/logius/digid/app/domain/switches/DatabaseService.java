...
public abstract class DatabaseService<T extends SQLObject> implements ApplicationListener<ContextRefreshedEvent> {
    ...
    @Value("${enable_switch_sync}")
    private boolean enableSwitchSync;

    protected DatabaseService(JpaRepository<T, Long> repository) {
    ...
    private void synchronizeObjects() throws IOException {
        ...
        for (Long id : existing) {
            ...
            if (!changes.isEmpty()) {
                ...
                logger.info("Updated {} with ID {}, changes: {}", type, id, changes);
            }
        }
    }
