...
import nl.logius.digid.app.domain.authentication.flow.AuthenticationFlow;
...
public class MijnDigidFlow extends AuthenticationFlow {
    public static final String NAME = "mijn_digid";

    private static final Map<BaseState, Map<BaseAction, BaseState>> TRANSITIONS_START_ACTION_RESULT = Map.of(
        State.INITIALIZED, Map.of(Action.CREATE_MANAGE_SESSION, State.AUTHENTICATED),
        State.AUTHENTICATED, Map.of(Action.GET_STATUS, State.AUTHENTICATED)
    );

    public MijnDigidFlow(AppAuthenticatorService appAuthenticatorService, DigidClient digidClient, NsClient nsClient, AppSessionService appSessionService, FlowFactory flowFactory) {
