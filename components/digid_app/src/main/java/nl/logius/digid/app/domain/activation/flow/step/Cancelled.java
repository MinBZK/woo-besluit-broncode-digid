...
public class Cancelled extends AbstractFlowStep<CancelFlowRequest> {
    private static final String ABORT_PARAM = "ABORT";

    private final DigidClient digidClient;

    public Cancelled(DigidClient digidClient) {
        super();
    ...
    public AppResponse process(Flow flow, CancelFlowRequest request) {
        String logNumber = (request.getCode() != null && request.getCode().equalsIgnoreCase(ABORT_PARAM)) ? "uc5.app_activation_by_letter_aborted" : "737";

        digidClient.remoteLog(logNumber, Map.of(
            lowerUnderscore(ACCOUNT_ID), appSession.getAccountId() != null ? appSession.getAccountId() : "",
            lowerUnderscore(APP_CODE), appSession.getAppCode() != null ? appSession.getAppCode() : "",
            lowerUnderscore(DEVICE_NAME), appSession.getDeviceName() != null ? appSession.getDeviceName() : ""));
