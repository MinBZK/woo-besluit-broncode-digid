...
public class CancelRda extends AbstractFlowStep<AppSessionRequest> {

    private final DigidClient digidClient;
    private final RdaClient rdaClient;

    public CancelRda(DigidClient digidClient, RdaClient rdaClient) {
        super();
    ...
    public AppResponse process(Flow flow, AppSessionRequest request) {
        if (appSession.getRdaAction().equals("upgrade_rda_widchecker")){
            digidClient.remoteLog("1311",
                Map.of(lowerUnderscore(ACCOUNT_ID), appSession.getAccountId(), HIDDEN, true));
        } else {
            digidClient.remoteLog("879", Map.of(lowerUnderscore(ACCOUNT_ID), appSession.getAccountId()));
        }
