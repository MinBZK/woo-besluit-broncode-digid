...
import nl.logius.digid.app.domain.authentication.response.StartSessionResponse;
...
public class Created extends AbstractFlowStep<AppRequest> {
    ...
    public AppResponse process(Flow flow, AppRequest request) throws FlowNotDefinedException, IOException, NoSuchAlgorithmException, SharedServiceClientException {
        appSession = new AppSession("login");
        appSession.setFlow(flow.getName());

        return new StartSessionResponse(appSession.getId(), Instant.now().getEpochSecond());
    }
