...
public class SmsSent extends AbstractFlowStep<AppSessionRequest> {

    private static final String SMS_TOO_FAST = "sms_too_fast";

    protected DigidClient digidClient;

    public SmsSent(DigidClient digidClient) {
        super();
        this.digidClient = digidClient;
    }

    @Override
    public AppResponse process(Flow flow, AppSessionRequest params) {

        if(appSession.getActivationMethod().equals(ActivationMethod.SMS))
            digidClient.remoteLog("1436", getAppDetails());

        return sendAndValidateSms();
    }

    public AppResponse sendAndValidateSms() {
        Map<String, String> result = digidClient.sendSms(appSession.getAccountId(), appSession.getActivationMethod(), null);
