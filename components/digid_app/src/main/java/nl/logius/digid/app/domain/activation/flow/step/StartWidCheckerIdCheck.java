...
import nl.logius.digid.app.domain.activation.flow.flows.WidCheckerIdCheckFlow;
...
public class StartWidCheckerIdCheck extends AbstractFlowStep<AppRequest> {

    private final DigidClient digidClient;

    public StartWidCheckerIdCheck(DigidClient digidClient) {
        super();
    ...
    public AppResponse process(Flow flow, AppRequest request) {
        ...
        appSession.setFlow(WidCheckerIdCheckFlow.NAME);
        appSession.setAction("upgrade_rda_widchecker");
        appSession.setRdaAction("upgrade_rda_widchecker");

        digidClient.remoteLog("1307", Map.of());
