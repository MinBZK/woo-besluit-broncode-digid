...
import nl.logius.digid.app.domain.activation.response.PollBrpResponse;
...
public class PollBrp extends AbstractFlowStep<AppSessionRequest> {
    private final DigidClient digidClient;

    public PollBrp(DigidClient digidClient) {
        super();
    ...
    public AppResponse process(Flow flow, AppSessionRequest request) {
        ...
        if (result.get(lowerUnderscore(STATUS)).equals("OK")) {
            ...
            setValid(false); // gba check in progress, do not transition to next step yet
            return new StatusResponse("PENDING");
        } else {
            if (result.get(lowerUnderscore(STATUS)).equals("NOK") && result.get(ERROR) != null) {
                return new PollBrpResponse((String) result.get(ERROR), result);
            }
