...
public class AwaitingDocuments extends AbstractFlowStep<AppRequest> {

    private DigidClient digidClient;

    public AwaitingDocuments(DigidClient digidClient) {
        super();
    ...
    public AppResponse process(Flow flow, AppRequest body) {
        Map<String, String> result = digidClient.requestWid(appSession.getAccountId());

        appSession.setWidRequestId(result.get(lowerUnderscore(WID_REQUEST_ID)));
        appSession.setRdaSessionStatus("AWAITING_DOCUMENTS");
