...
/*------------------------------------------------------------------------
    Inner part of SignedPIP, the payload
    signedPIP SEQUENCE {
        pip PIP,
...
------------------------------------------------------------------------*/
public class USvESignedPIPPayload {
    ...
    public static USvESignedPIPPayload decode(byte[] encoded) throws Exception {
        ...
        USvESignedPIPPayload spip = new USvESignedPIPPayload();
                
        spip.encodedData = encoded;

        spip.pip = USvEPIP.decode(elements[0].toASN1Primitive().getEncoded());
        spip.auditElement = ((DEROctetString) elements[1].toASN1Primitive()).getOctets();
        spip.signingKeyVersion = ((ASN1Integer) elements[2]).getValue().intValue();

        if(elements.length>3) {
            spip.issuanceDate = ((DERIA5String) elements[3]).getString();
        }
