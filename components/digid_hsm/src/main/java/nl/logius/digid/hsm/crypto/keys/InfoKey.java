...
public class InfoKey extends KeysAction {
    private final Logger logger = LoggerFactory.getLogger(getClass());

    public InfoKey(ConnectionPool pool) {
        super(pool, false);
    }

    public KeyInfoResponse info(String group, String name) {
        final KeyInfoResponse response = new KeyInfoResponse();
        try {
            response.setPublicKey(getConnection().publicKey(group, name));
        } catch (IOException | CryptoServerException e) {
            setLastException(e);
            logger.error(String.format("Error getting info of key <%s,%s>", group, name), e);
