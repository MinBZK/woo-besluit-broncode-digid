...
public class WebServiceTestConfig {
    ...
    @Bean
    public RegisterSCStatusPort registerSCStatusPortService() {
        JaxWsProxyFactoryBean jaxWsFactory = new JaxWsProxyFactoryBean();
        jaxWsFactory.setServiceClass(RegisterSCStatusPort.class);
        jaxWsFactory.setAddress("http://localhost:8090" + WebServiceConfig.BASE_URL + WebServiceConfig.SERVICE_URL);
        jaxWsFactory.getOutFaultInterceptors().add(logOutInterceptor());
        jaxWsFactory.getOutInterceptors().add(logOutInterceptor());
        return (RegisterSCStatusPort) jaxWsFactory.create();
    }
