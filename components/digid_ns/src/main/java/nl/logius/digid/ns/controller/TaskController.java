...
import nl.logius.digid.ns.model.ScheduledTask;
...
public class TaskController {
    ...
    @Autowired
    public TaskController(NotificationService notificationService) {
    ...
    public ResponseEntity<Map<String, Object>> performScheduleTask(final @RequestBody ScheduledTask task) {
        if(task.getIapi().get("name").equalsIgnoreCase("resend_unsent_pushnotifications")) {
            notificationservice.resendNotifications();
        } else if (task.getIapi().get("name").equalsIgnoreCase("clean_up_notifications")) {
