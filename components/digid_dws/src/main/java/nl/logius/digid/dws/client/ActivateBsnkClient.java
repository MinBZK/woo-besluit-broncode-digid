...
import eid.gdi.nl._1_0.webservices.BSNKActivatePort;
import eid.gdi.nl._1_0.webservices.BSNKActivateService;
import eid.gdi.nl._1_0.webservices.BSNKProvidePolymorphicFault;
...
@Component
public class ActivateBsnkClient extends AbstractBsnkClient {

    @Autowired
    public ActivateBsnkClient(@Value("${ws.client.connection_timeout}") Integer connectionTimeout,
            ...
            @Value("${ws.client.bsnk_activate.endpoint}") String endpoint,
    ...
    public List<PolymorphicPseudonymType> providePPRequest(ProvidePPPPCAOptimizedRequest request) throws BsnkException {
        try {
            return ((BSNKActivatePort) this.bindingProvider).bsnkProvidePPPPCAOptimized(request)
                    .getPolymorphicPseudonyms();
        } catch (BSNKProvidePolymorphicFault ex) {
            throw new BsnkException(ex);
            ...
            throw new BsnkException("BsnkActivateSoapFault", ex.getMessage(), ex);
    ...
    protected BindingProvider getBsnkPort() {
        BSNKActivateService service = new BSNKActivateService(new WSAddressingFeature());
        return (BindingProvider) service.getBSNKActivate();
    }
}
