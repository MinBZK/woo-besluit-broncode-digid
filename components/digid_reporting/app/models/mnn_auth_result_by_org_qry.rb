...
class MnnAuthResultByOrgQry < MnnBaseQry

  def initialize(type:, organization:)
    super(name: "MnnAuthResultByOrgQry_#{RESULT_TYPES[type]}_#{organization.name}", type: type, filter_by_webservices: true)
    @webservice_ids = get_webservice_ids_by_organisation(organization)
  end
  ...
  def get_webservice_ids_by_organisation(organization)
    webservices = Webservice.where(:organization_id => organization.id)
    webservice_ids = []
