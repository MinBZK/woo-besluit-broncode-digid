...
class ReportSectorTooManyAuthFraud < AdminReport
    ...
    @@threshold = APP_CONFIG['auth_per_sectorno_threshold']

  def self.report_name
    "Sectoraalnummers met meer dan #{@@threshold} authenticaties"
  end
  ...
  def self.report(start_date = nil)
    ...
    header = ["Periode", "Sector number", "SSSSSSSSSSSSSSSSSSSSS"]
    ...
    values = Log.from(from).where(created_at: (start_ts..end_ts), code: lookup_codes(@@uc_labels)).
                 group(:sector_number).having("count_all > ?", @@threshold).count
