...
class ReportPostcodeTooManyRegFraud < AdminReport
  ...
  def self.report(start_date = nil)
    ...
    header = ["Periode", "Postcode", "SSSSSSSSSSSSSSSSSSSSS"]
    status = ActivationLetter::Status::SEND_TO_PRINTER

    logger.info "#{report_name} ===> Generating report for #{start_ts} .. #{end_ts}"

    values = ActivationLetter.where(:created_at => (start_ts..end_ts), :status => status).group(:postcode).having("count_all > ?", APP_CONFIG['auth_per_postcode_threshold']).count
