...
class ReportSuccessAuthOrgYearDcMonthly < AdminReport
  ...
  def self.report_name
    "Aantal gelukte authenticaties per organisatie per jaar tbv doorbelasting - Dienstencatalogus gedetailleerd"
  end
  ...
  def self.report (start_date = nil)
    ...
    result = [["Vanaf maand", "T/m maand", "Organisatie", "Organisatie ID", "Actief/inactief", "Organisatierol ID", "Organisatierol", "Dienst ID", "Dienst", "Totaal aantal authenticaties dienst"]]
    ...
    Dc::Service.each_in_batches do |service|
      count_per_connection_per_service[service.connection_id] ||= []
      count_per_connection_per_service[service.connection_id]<< {id: service.id, name: service.name, count: count_data[service.legacy_service_id] || 0}
    end
    ...
    Dc::Organization.each_in_batches do |organization|
      ...
      organization.organization_roles.each do |role|
        role_data = organization_data + [role.id, role.type]

        count_per_organization_role[role.id]&.map(&:second)&.flatten&.map(&:values)&.each do |service_data|
          result << role_data + service_data
        end
      end
    end
