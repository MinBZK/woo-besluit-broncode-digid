...
class MnnAuthsQry < MnnBaseQry

  def initialize(type:)
    super(name: "MnnAuthsQry_#{RESULT_TYPES[type]}", type: type)
  end

  def self.get_types
    return [ :success, :failure, :attempt ]
  end
  ...
  def get_failure_codes
    codes_failure = [
      ...
      "uc2.authenticeren_midden_mislukt_sms_code_onjuist",
      "uc2.authenticeren_midden_mislukt_sms_ongeldig"
    ]
  end

  def get_attempt_codes
    codes_start = [
      "uc2.authenticeren_start",
      "uc2.authenticeren_start_basis",
      "uc2.authenticeren_start_midden"
    ]
  end
end
