...
public abstract class AccountController {
    ...
    public AccountController(MijnDigidSessionRepository mijnDigiDSessionRepository) {
        this.mijnDigiDSessionRepository = mijnDigiDSessionRepository;
    }

    protected MijnDigidSession retrieveMijnDigiDSession(String mijnDigiDsessionId) {
        Optional<MijnDigidSession> optional = this.mijnDigiDSessionRepository.findById(mijnDigiDsessionId);
        if (optional.isEmpty()) {
            throw new RuntimeException("session should always be available!");
        }
        return optional.get();
    }
