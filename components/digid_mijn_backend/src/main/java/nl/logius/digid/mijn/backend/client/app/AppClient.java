...
public class AppClient extends IapiClient {
    ...
    public AppClient(HttpUrl baseUrl, String authToken, int timeout) {
        super(baseUrl, authToken, timeout);
    }

    public Optional<AppSession> getAppSession(String appSessionId) {
        try {
            return Optional.of(execute("sessions/"+appSessionId, Map.of(), AppSession.class, HttpMethod.GET));
        } catch (ClientException e) {
            logger.error("Connection error while trying to connect to digid_app: {}", e.getMessage());
            throw new AppRuntimeException("Connection error while trying to connect to digid_app", e);
        }
    }
}
