...
public class DTwoFactorStatusResult extends AccountResult {

    @JsonInclude(NON_NULL)
    @JsonProperty("setting_2_factor")
    private Boolean setting;

    public static DTwoFactorStatusResult copyFrom(TwoFactorStatusResult result) {
        DTwoFactorStatusResult deprecatedResult = new DTwoFactorStatusResult();
        deprecatedResult.status = result.getStatus();
        deprecatedResult.error = result.getError();
        deprecatedResult.setting = result.getSetting();
