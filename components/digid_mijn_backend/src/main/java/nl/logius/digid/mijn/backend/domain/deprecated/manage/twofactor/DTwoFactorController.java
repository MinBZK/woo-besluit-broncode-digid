...
@RestController
@RequestMapping("/apps/two_factor")
public class DTwoFactorController extends DAccountController {

    private AccountService accountService;

    @Autowired
    public DTwoFactorController(AppClient appClient, AccountService accountService) {
        super(appClient);
        this.accountService = accountService;
    }

    @PostMapping("/get_two_factor")
    @Operation(summary = "Get the two factor setting of an account")
    public DTwoFactorStatusResult getTwoFactor(@RequestBody DAccountRequest deprecatedRequest) {
        AppSession appSession = validate(deprecatedRequest);

        var result =  accountService.getTwoFactorStatus(appSession.getAccountId(), appSession.getDeviceName(), appSession.getAppCode());
        return DTwoFactorStatusResult.copyFrom(result);
    }
    ...
    private void validateSettingTwoFactor(DTwoFactorChangeRequest deprecatedRequest){
        if(deprecatedRequest.getSettingTwoFactor() == null) {
