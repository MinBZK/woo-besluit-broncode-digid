...
module RegistrationChecks
  class TooSoon < RegistrationCheck
    attr_reader :session
    ...
    def match?
      !@session[:balie] && @registration.registration_too_soon?("snelheid_aanvragen", %w(valid emigrated rni ministerial_decree))
    end
    ...
    def flash_msg(options = {})
      ...
      if last_registration.status_emigrated_or_rni_or_ministerial_decree?
        time_calc = last_registration.created_at + ::Configuration.get_int("snelheid_aanvragen").days
