      ...
      module IdentityCard
        class ExistingUnblockLetterRequestsController < ApplicationController
            ...
            def show
              ...
              session[:existing_unblock_letter_request_id_card] = true

              Log.instrument("1011", wid_type: t("document.id_card", locale: :nl), account_id: current_account.id)

              begin
                @identity_card = rvig_client.get(bsn: current_account.bsn, sequence_no: session[:sequence_no]).first
                if !@identity_card.deblockable?
              ...
              if !current_account.unblock_letter_requested?(bsn: current_account.bsn, sequence_no: params[:sequence_no], card_type: params[:card_type])
                Log.instrument("1010", wid_type: t("document.id_card", locale: :nl), account_id: current_account.id, hidden: true)
                flash.now[:notice] = t("digid_hoog.request_unblock_letter.no_pending_request", wid_type: t("document.id_card")).html_safe
                render_simple_message(ok: my_digid_url)
              end
            end

            def cancel
              Log.instrument("1012", wid_type: t("document.id_card", locale: :nl), account_id: current_account.id)
