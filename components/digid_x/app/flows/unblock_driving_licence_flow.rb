...
class UnblockDrivingLicenceFlow
    # initialize > enter_code > choose_card_reader > app_chosen > scan_qr_code > qr_code_scanned > scan_wid > wid_scanned > completed
    FLOW = {
    # Step 1 - Enter code from letter
    enter_code:           { transitions: [:verify_code, :failed] },
    verify_code:          { transitions: [:choose_card_reader, :failed]},

    # Step 2 - choose card reader
