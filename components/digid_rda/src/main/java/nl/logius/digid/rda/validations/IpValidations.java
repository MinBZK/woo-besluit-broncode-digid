...
package nl.logius.digid.rda.validations;
...
public class IpValidations {
    ...
    public void ipCheck(RdaSession session, String clientIp) {
        if (session.getClientIpAddress() != null && !session.getClientIpAddress().isEmpty()) {
            ...
            if (!anonimizedIp.equals(session.getClientIpAddress())) {
                String logMessage = String.format(
                    "Security exception: IP of DigiD app from X doesn't match current IP of DigiD app: %s expected: %s",
