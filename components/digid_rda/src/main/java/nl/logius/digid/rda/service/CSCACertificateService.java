    ...
    public CRL add(X509CRL crl) {
        ...
        if (old.isPresent()) {
            if (!db.getThisUpdate().isAfter(old.get().getThisUpdate())) {
                throw new BadRequestException("CRL is not newer, refuse to update");
            }
        ...
        if (!dbCert.isPresent()) {
            ...
            throw new BadRequestException("Could not get certificate to verify");
        }
        ...
        try {
            ...
            throw new BadRequestException("Could not verify CRL", e);
        }
